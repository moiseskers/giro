...
@Component({
    selector: 'app-p-steps',
    template: `
        <nav [ngClass]="{ 'p-steps p-component': true }" [ngStyle]="style" [class]="styleClass" [attr.data-pc-name]="'steps'">
            <ul #list role="tablist" [attr.data-pc-section]="'menu'">
                <li [ngStyle]="{ 'justify-content': i === 0 ? 'start' : (i === 1 ? 'center' : 'flex-end') }"
                    *ngFor="let item of model; let i = index" 
                    ...
                    [attr.aria-current]="isActive(item, i, model) ? 'step' : undefined"
                    role="presentation"
                    [attr.id]="item.id"
                    pTooltip
                    [tooltipOptions]="item.tooltipOptions"
                    [ngClass]="{ 
                        'p-highlight p-steps-current ': isActive(item, i, model)
               
                     }"
                    [attr.data-pc-section]="'menuitem'"
                >
                    <a [ngStyle]="{ 'align-items': i === 0 ? 'start' : (i === 1 ? 'center' : 'end') }"
...
export class Steps implements OnInit, OnDestroy {
    ...
    isActive(item: MenuItem, index: number, array: any[]) {
        // which route am I?
        // get the position of this route inside the array
        let currentActivatedIndex = array.map((value, index) => value.routerLink === this.router.url ? index : -1).filter(index => index !== -1)[0];

        // if the position is not 0, activate all routes bellow it
