...
@Component({
    ...
    template: `
        <div #container [ngClass]="containerClass" [ngStyle]="style" [class]="styleClass"
             (click)="onContainerClick($event)" class="input-container">
            ...
            <ng-container *ngIf="loading">
                <SpinnerIcon *ngIf="!loadingIconTemplate" class="p-autocomplete-loader" [spin]="true"
                             [attr.aria-hidden]="true"/>
                <span *ngIf="loadingIconTemplate" class="p-autocomplete-loader pi-spin " [attr.aria-hidden]="true">
                    <ng-template *ngTemplateOutlet="loadingIconTemplate"></ng-template>
                </span>
            </ng-container>
            <button #ddBtn type="button" pButton [attr.aria-label]="dropdownAriaLabel"
                    [ngClass]="
                    (
                    ...
                    ?
                    dropdownIcon + ' ng-invalid ng-touched ng-dirty' :
                    dropdownIcon
                    "

                    class="p-autocomplete-dropdown p-button-icon-only bg-transparent border-left-0 left-button"

                    [disabled]="disabled" pRipple
            ...
            <p-overlay
                    ...
                    (onBeforeHide)="overlayOnBeforeHideEvent($event)"
                    (onHide)="hide()"
            >
                <div [ngClass]="panelClass" [style.max-height]="virtualScroll ? 'auto' : scrollHeight"
                    ...
                    <ng-template #buildInItems let-items let-scrollerOptions="options">
                        <ul #items class="p-autocomplete-items" [ngClass]="scrollerOptions.contentStyleClass"
                            [style]="scrollerOptions.contentStyle" role="listbox" [attr.id]="id + '_list'"
                            ...
                            <ng-template ngFor let-option [ngForOf]="items" let-i="index">
                                ...
                                <ng-container *ngIf="multiselect">
                                    ...
                                    <li
                                            ...
                                            (click)="onOptionMultiSelect($event, option)"
                                        ...
                                        <div class="flex gap-1 align-items-center">
                                            <div class="p-checkbox p-component">
                                                <div class="p-checkbox-box"
                                                     [ngClass]="{ 'p-highlight': isSelectedFound(option) }">
                                                    <ng-container *ngIf="isSelectedFound(option)">
                                                        <CheckIcon
                                                                [styleClass]="'p-checkbox-icon'"
                                                                [attr.aria-hidden]="true"/>
                                                        <span class="p-checkbox-icon" [attr.aria-hidden]="true"></span>
                                                    </ng-container>
                                                </div>
                                            </div>
                                ...
                                <ng-container *ngIf="!isOptionGroup(option) && !multiselect">
                            ...
                            <li *ngIf="!items || (items && items.length === 0 && showEmptyMessage)"
        ...
        <div class="flex gap-2 flex-wrap mt-2" *ngIf="multiple">
            <ng-container *ngFor="let option of modelValue(); let i = index">
                <div class="description">
                    <p-chip (onRemove)="removeOption($event, i); removeOptionMultiSelect(option)" [label]="getOptionLabel(option)" [removable]="true"
                            styleClass="border-solid border-1 border-primary w-chip"></p-chip>
                </div>
            </ng-container>
        </div>
    ...
    styleUrls: ['./app-auto-complete.component.css']
})
export class AppAutoComplete implements AfterViewChecked, AfterContentInit, OnDestroy, ControlValueAccessor {
    ...
    @ViewChild('focusInput') inputElement!: ElementRef<HTMLInputElement>;
    isDisabled = false; // Example to toggle disabled state
    ...
    constructor(
        ...
        @Inject(DOCUMENT) private document: Document, public el: ElementRef, public renderer: Renderer2, public cd: ChangeDetectorRef, public config: PrimeNGConfig, public overlayService: OverlayService, private zone: NgZone) {
        effect(() => {
    ...
    get multiContainerClass() {
        // return 'p-autocomplete-multiple-container p-component p-inputtext w-full' ;


        return {
            'p-autocomplete-multiple-container p-component p-inputtext w-full': true,
    ...
    get panelClass() {
        return {
            ...
            'p-input-filled': this.config.inputStyle ===  signal('filled'),
    ...
    get inputClass() {
        return {
            ...
            'ng-invalid ng-touched ng-dirty': (!this.ngControl?.control?.pending && !this.ngControl?.control?.valid && this.ngControl?.control?.touched && this.ngControl?.control?.errors),
        };
    }
    ...
    onOptionMultiSelect(event, option) {

        let found = this.isSelectedFound(option);
        ...
        if (found) {
            this.selected = this.selected.filter(item => JSON.stringify(item) !== JSON.stringify(option))
        } else {
            this.selected.push(option);
        }
    }

    _onOptionMultiSelect(event, option) {
    ...
    search(event: any, query: string, source: string) {
    ...
    overlayOnBeforeHideEvent($event: OverlayOnBeforeHideEvent) {
        if (this.multiselect) {
            this.selected.forEach(value => {
                this._onOptionMultiSelect($event, value)
            });
        }

    }

    isSelectedFound(option: any) {
        return this.selected.some(item => JSON.stringify(item) === JSON.stringify(option));
    }

    removeOptionMultiSelect(option: any) {
        this.selected = this.selected.filter(item => JSON.stringify(item) !== JSON.stringify(option))
    }
...
@NgModule({
    imports: [CommonModule, OverlayModule, InputTextModule, ButtonModule, SharedModule, RippleModule, ScrollerModule, AutoFocusModule, TimesCircleIcon, SpinnerIcon, TimesIcon, ChevronDownIcon, ChipModule, CheckIcon, CheckboxModule],
    exports: [AppAutoComplete, OverlayModule, SharedModule, ScrollerModule, AutoFocusModule],
    declarations: [AppAutoComplete]
})
