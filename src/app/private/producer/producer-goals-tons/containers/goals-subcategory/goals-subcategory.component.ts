...
export class GoalsSubcategoryComponent implements OnInit {
    ...
    private async loadChartData(categoryId: 'DOMICILIARY' | 'NON_DOMICILIARY', year: string, chartLoaderId: string, chartType: 'domiciliary' | 'noDomiciliary') {
        let response;
        ...
        const hasData = response.items && response.items.length;
        const labels = hasData ? response.items.map((value) => value.subcategory) : ['Sem Dados'];
        const colors = hasData ? this.getChartColors(labels) : ['#ccc'];
        ...
        const chartData = {
            ...
            datasets: [{
                ...
                borderColor: colors,
                data: hasData ? response.items.map((value) => value.goal) : [0.00],
            }]
        };
