    ...
    template: `
        <ng-container>
            ...
            >
                ...
                <span *ngIf="item?.materialIcon" [ngClass]="item.materialIcon" class="material-symbols-outlined layout-menuitem-icon material-font-size">{{item.materialIcon}}</span>
                <span class="layout-menuitem-text ">{{ item.label }}</span>
                <i class="pi pi-fw pi-angle-down layout-submenu-toggler" *ngIf="item.items"></i>
            </a>
            
            <a class="menu-item" *ngIf="item.routerLink && !item.items && item.visible !== false" (click)="itemClick($event)" (mouseenter)="onMouseEnter()" [ngClass]="item.class" [routerLink]="item.routerLink" routerLinkActive="bg icon-active" [routerLinkActiveOptions]="item.routerLinkActiveOptions || { paths: 'exact', queryParams: 'ignored', matrixParams: 'ignored', fragment: 'ignored' }" [fragment]="item.fragment" [queryParamsHandling]="item.queryParamsHandling" [preserveFragment]="item.preserveFragment" [skipLocationChange]="item.skipLocationChange" [replaceUrl]="item.replaceUrl" [state]="item.state" [queryParams]="item.queryParams" [attr.target]="item.target" tabindex="0" pRipple [pTooltip]="item.label"  [tooltipDisabled]="!(isSlim && root)">

                <div class="flex">
                    <ng-container>
                        <div *ngIf="item?.icon" [ngClass]="item.icon" class="layout-menuitem-icon icon"></div>
                        <div *ngIf="item?.materialIcon" [ngClass]="item.materialIcon"
                             class="material-symbols-outlined layout-menuitem-icon material-font-size icon">{{ item.materialIcon }}
                        </div>
                    </ng-container>

                    <div class="layout-menuitem-text text ">{{ item.label }}</div>
                </div>
